<div align="center">
  <!-- Subtle Banner Animation -->
  <img height="180" src="https://media.giphy.com/media/qgQUggAC3Pfv687qPC/giphy.gif" />
</div>

# 👋 Govind Sai Krishna Rayarao  
**Python Backend Developer | Hyderabad 🇮🇳**

[![Python](https://img.shields.io/badge/Python-3.11-blue?style=flat-square&logo=python)](https://www.python.org/)
[![Django](https://img.shields.io/badge/Django-REST-green?style=flat-square&logo=django)](https://www.djangoproject.com/)
[![License](https://img.shields.io/badge/License-MIT-green?style=flat-square)](LICENSE)

---

## 🧠 About Me

I am an entry-level **Python Backend Developer** focused on building **robust, scalable, and maintainable backend systems** using proven engineering practices.

- 🐍 Strong foundation in **Python & Django**
- 🔭 Former **Project Intern @ BHEL**
- ⚙️ Built automation pipelines reducing manual work by **~80%**
- 🌱 Actively working with **Django REST Framework**
- ☁️ **Microsoft Azure Fundamentals (AZ-900)** certified
- 🧩 Advocate of clean architecture, readable code, and long-term scalability

> Straight talk:  
> Code that only works today is technical debt in disguise.

---

## 🛠 Tech Stack

<div align="center">
  <img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/python/python-original.svg" height="35"/>
  <img width="15"/>
  <img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/django/django-plain.svg" height="35"/>
  <img width="15"/>
  <img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/postgresql/postgresql-original.svg" height="35"/>
  <img width="15"/>
  <img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/mysql/mysql-original.svg" height="35"/>
  <img width="15"/>
  <img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/git/git-original.svg" height="35"/>
  <img width="15"/>
  <img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/linux/linux-original.svg" height="35"/>
</div>

---

## 🧪 Featured Projects

### 🔧 Automated Data Extraction Pipeline
- Python-based **ETL automation pipeline**
- Combines **web scraping + OCR**
- Reduced processing time by **~80%**
- Tools: Python, OpenCV, Pytesseract, BeautifulSoup  

### 🏥 Hospital Management System
- Full-stack Django application
- REST APIs with role-based access control
- PostgreSQL-backed relational database

### 📰 Fake News Detection System
- Machine Learning + NLP project
- TF-IDF feature extraction with classifiers
- Focused on preprocessing, evaluation, and accuracy

---

## 📊 GitHub Stats

<p align="center">
  <img src="https://github-readme-stats.vercel.app/api?username=GovindSaiKrishnaRayarao&show_icons=true&theme=dracula&hide_border=true" height="170" />
  <img src="https://github-readme-stats.vercel.app/api/top-langs/?username=GovindSaiKrishnaRayarao&layout=compact&theme=dracula&hide_border=true" height="170" />
</p>

<p align="center">
  <img src="https://github-readme-streak-stats.herokuapp.com/?user=GovindSaiKrishnaRayarao&theme=dracula&hide_border=true" alt="GitHub Streak" />
</p>

---

## 🌐 Connect With Me

<div align="center">
  <a href="https://www.linkedin.com/in/govind-sai-krishna-rayarao">
    <img src="https://img.shields.io/badge/LinkedIn-0077B5?style=flat-square&logo=linkedin"/>
  </a>
  <a href="mailto:rayaraogovindsaikrishna@gmail.com">
    <img src="https://img.shields.io/badge/Gmail-D14836?style=flat-square&logo=gmail"/>
  </a>
  <a href="https://github.com/GovindSaiKrishnaRayarao">
    <img src="https://img.shields.io/badge/GitHub-181717?style=flat-square&logo=github"/>
  </a>
</div>

---

## ⚡ Contribution & Collaboration

- Open to **backend roles**, **internships**, and **open-source**
- Feel free to raise issues or submit pull requests
- ⭐ Star repositories if they add value
